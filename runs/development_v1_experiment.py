...
class Environment:
 def __init__(self,task,seed,split='test',overrides=None):
  ...
  self.initial=int(rng.integers(2));self.noise=rng.random((HORIZON*TICKS,2));self.outcomes=rng.random((HORIZON,2))
 ...
 def advance(self):
  ...
  for tick in range(self.t*TICKS,(self.t+1)*TICKS):
   ...
   self.values=([3.,1.5] if hot==0 else [1.5,3.]);self.costs=([2.,1.] if hot==0 else [1.,2.])
...
def llm(env,signal,tag):
 ...
 user={'rules':RULES[env.task],'observations':env.observations,'history':env.history,'controller':{'recommendation':signal['recommendation'],'scores':signal['scores']}}
...
if __name__=='__main__':
 ...
 if args.command=='graphs':print(make_graphs())
  ...
  is_test=args.command=='test';tag='test' if is_test else 'validation';path=ROOT/f'runs/{tag}_episodes.jsonl'
  ...
  jobs=[(task,30000+i if is_test else 21000,seed,condition) for task in TASKS for i in range(3 if is_test else 1) for seed in (SEEDS if is_test else [11]) for condition in CONDITIONS]
